...
export default {
  ...
  template: `
    <div class="ten columns createevent" >
        <OAuthLogin />
        <div v-show="!$store.state.displayLoginButton">
  ...
  methods: {
    ...
    getTimes: async function() {
      let dates = await fetch(
        `/api/v1/calendar?duration=${this.runtime.replace(" min", "")}`
      );
      dates = await dates.json();
      let today = new Date();
      this.calendarOpenings = dates.map(
        date => date - today.getTimezoneOffset() * 60 * 1000
      );
    },
